...
| NFR-REL-002 | Partial | `ControlServer::stop()` joins TCP/UDP workers, clears keys/samples and withdraws owned DNS-SD (`native/vcam-net/src/control.rs:282`, `discovery.rs:49`). DNS-SD shutdown acknowledgement is bounded to 500 ms and errors are returned; stopped servers cannot advertise (`native/vcam-net/tests/control_server.rs:496`). Real multicast smoke measured 38 ms stop, observed both goodbye removals, re-enabled on the same ports, and verified drop withdrawal (`docs/LOOP_LOG.md`, iteration 26). Blender unregister wiring remains 1.2.5/1.3.x. |
