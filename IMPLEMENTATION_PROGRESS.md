...
| FR-BL-002 | Partial | The native module owns the sockets and threads (`vcam-net` `ControlServer`/`UdpReceiver`, tasks 1.2.1–1.2.4). They are now exposed to Python as `vcam_native.Session` (`native/vcam-py/src/lib.rs:82`, task 1.2.5a): `start`/`stop` release the GIL; `latest_pose`, `stats`, `poll_event` and `pairing_code` are non-blocking reads of latest-sample slots and the event queue (C-2). Verified in headless Blender 5.2.2 (`tests/blender/session_native.py`). Not yet used by the add-on: register/unregister wiring is 1.2.5b, and the add-on still uses the Python FreeD client (`core/udp_client.py`) until 1.3.1. |
