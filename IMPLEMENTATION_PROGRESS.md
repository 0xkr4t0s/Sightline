...
| NET-004 | Partial | Host reconnect replaces the authenticated TCP/UDP session with fresh keys, closes old TCP, rejects old-key UDP and resets samples/sequence filters (`native/vcam-net/tests/control_server.rs:346`, task 1.2.2b.2a). Reconnect also succeeds after the 10-second authenticated-datagram idle timeout (`:209`). The remembered authenticated UDP source follows roaming; outbound replies remain 1.2.2b.2b. Still to do: device-side retry and the 3 s reconnect measurement (1.4.x), `.blend` reload (1.3.4). |
...
| NFR-REL-002 | Partial | `ControlServer::stop()` joins TCP/UDP workers, closes sockets and clears keys/samples (`native/vcam-net/src/control.rs:242`, task 1.2.2b.2a). A real active session plus idle TCP connections stop in under 1 s, and a new server authenticates and receives UDP on the same TCP/UDP ports while the stopped handle remains alive (`native/vcam-net/tests/control_server.rs:470`). The Blender unregister path isn't wired yet (1.2.5/1.3.x). |
| NFR-REL-003 | Not started | |
| NFR-SEC-001 | Partial | Rust: SRP-6a, HKDF session keys, constant-time proofs, authenticated UDP, single-use/expiring codes and failed-proof lockout. File-backed pairing keys: `native/vcam-net/src/store.rs:37-126` (atomic snapshot replacement, Unix 0700/0600 permissions, malformed stores rejected); persistence precedes `PAIR_ACCEPT` (`native/vcam-net/src/control.rs:423-438`). Storage-failure/retry and private-file tests: `native/vcam-net/tests/control_server.rs:601`, `:735`. UDP keys activate only after a verified TCP session and are revoked on replacement, disconnect, 10-second authenticated-datagram idle expiry or shutdown (`native/vcam-net/src/udp.rs:122`, `:241`, `:261`, `:302`; real TCP/UDP tests `control_server.rs:174`, `:209`, `:274`, `:346`, `:470`, task 1.2.2b.2a). Still to do: Blender config-directory and host-ID wiring (1.2.5), iOS Keychain (1.4.3). |
