...
| DM-004 | Partial | `testdata/coords/arkit_to_canonical.json` (task 1.1.2): identity, ±90° pan/tilt/roll, combined, translated. Each has ARKit and canonical position/quaternion/4×4 plus view and up directions; checked against independent matrix math (max diff 4.8e-10). Not yet consumed by Swift or Rust tests (1.1.3/1.1.4). |
...
| PR-001..003, PR-006 | Partial | Rust (`native/vcam-protocol`): UDP framing, HMAC-SHA256/64 trailer, messages (`endpoint.rs`, `message.rs`, task 1.1.3a); TCP control frames `HELLO`/`PAIR_*`/`SESSION_*`/`ERROR` (`control.rs`), SRP-6a pairing and session key schedule (`pairing.rs`, task 1.1.3b). All byte-exact against `messages.json`, `pairing.json`, `session.json` (`tests/golden.rs`, `tests/pairing.rs`). Still to do: sockets (1.2.x), Swift (1.1.4). |
| PR-005 | Partial | `Endpoint::open` applies vcp.md §4.3 steps 1–8 in order with typed `DropReason`s. Decoding is bounds-checked (`src/wire.rs`), with no `unwrap` (clippy-enforced). It matches all 26 `receive.json` verdicts, and a prefix/bit-flip/20k-random sweep neither panics nor authenticates. Fuzz targets are 1.1.3c. |
...
| NFR-QA-003 | Partial | `testdata/` holds VCP message, receive-rule, pairing/session, and coordinate vectors (task 1.1.2). There are no fragment-reassembly vectors yet (Phase 2). CI checks they're up to date (`ci.yml` job `python`: `tools/gen_testdata.py --check`). Rust/Swift/Python consumers come in 1.1.3/1.1.4/1.3.x. |
