...
| NFR-SEC-001 | Partial | Rust: SRP-6a, HKDF session keys, constant-time proofs, authenticated UDP, single-use/expiring codes and failed-proof lockout. File-backed pairing keys: `native/vcam-net/src/store.rs:37-126` (atomic snapshot replacement, Unix 0700/0600 permissions, malformed stores rejected); persistence precedes `PAIR_ACCEPT` (`native/vcam-net/src/control.rs:388-404`). Real storage-failure/retry and private-file tests: `native/vcam-net/tests/control_server.rs:420`, `:554`. Still to do: real Blender config-directory and host-ID wiring (1.2.5), control-session/UDP lifecycle wiring (1.2.2b.2), iOS Keychain (1.4.3). |
