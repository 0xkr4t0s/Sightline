...
| FR-VF-* | Not started | |
| FR-CTL-* | Not started | |
| FR-UX-001/002 | Partial | iOS: manual host/port entry only (`TrackingSettings.swift`); no discovery or pairing UI. Rust `ControlServer` handles code-based SRP pairing (1.2.2a). Pairings now survive host restart via `FileStore` (`native/vcam-net/src/store.rs:37`, task 1.2.2b.1), verified over TCP without another code (`native/vcam-net/tests/control_server.rs:602`). Still to do: Blender config-path/stable-host-ID wiring (1.2.5), N-panel code display (1.3.3), Swift pairing (1.1.4b, waiting for owner). |
...
| PR-001..003, PR-006 | Partial | Rust (`native/vcam-protocol`): UDP framing, HMAC trailer, messages, TCP control frames, SRP-6a pairing, session keys (1.1.3a/b). Swift (`VCamIOS/VCamIOS/VCP/VCPEndpoint.swift`, `VCPMessages.swift`, 1.1.4a): UDP framing, CryptoKit HMAC-SHA256/64 with constant-time compare, receive rules, POSE/CONTROL_STATE/CLOCK/STATUS; byte-exact golden vectors. Host networking now authenticates incoming pose/control and sends session-scoped STATUS/CLOCK (`native/vcam-net/src/udp.rs:218`, 1.2.2b.2b); real wire tests `native/vcam-net/tests/udp_receiver.rs:209`, `:246`, `:313`. Applied acknowledgements are explicitly published through `ControlServer::update_status` (`control.rs:212`), never inferred from reception. Still to do: Swift TCP/pairing (1.1.4b), device sockets (1.4.x), Blender bindings (1.2.5). |
...
| NFR-REL-002 | Partial | `ControlServer::stop()` joins TCP/UDP workers, closes sockets and clears keys/samples (`native/vcam-net/src/control.rs:247`, task 1.2.2b.2a). A real active session plus idle TCP connections stop in under 1 s, and a new server authenticates and receives UDP on the same TCP/UDP ports while the stopped handle remains alive (`native/vcam-net/tests/control_server.rs:496`). The Blender unregister path isn't wired yet (1.2.5/1.3.x). |
| NFR-REL-003 | Not started | |
| NFR-SEC-001 | Partial | Rust: SRP-6a, HKDF session keys, constant-time proofs, authenticated UDP, single-use/expiring codes and failed-proof lockout. Pairing keys persist atomically before PAIR_ACCEPT (`native/vcam-net/src/store.rs:37-126`, `control.rs:428-443`; storage-failure/private-file tests `native/vcam-net/tests/control_server.rs:627`, `:761`). Verified TCP sessions own UDP keys, revoked on replacement/disconnect/10-second idle expiry/shutdown (`udp.rs:166`, `:330`, `:354`, `:429`). Outbound STATUS/CLOCK uses those keys and only an authenticated source; stale-session status updates are refused (`udp.rs:218`, `:375`; tests `native/vcam-net/tests/udp_receiver.rs:246`, `:313`, task 1.2.2b.2b). Still to do: Blender config-directory/host-ID wiring (1.2.5), iOS Keychain (1.4.3). |
