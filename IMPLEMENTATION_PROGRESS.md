...
| NET-003 | Partial | Host sends authenticated CLOCK requests at 1 Hz using a monotonic worker clock (`native/vcam-net/src/udp.rs:218`, task 1.2.2b.2b; real timing test `native/vcam-net/tests/udp_receiver.rs:209`). Offset/delay math `ClockSample::from_timestamps` matches the golden vector. Reply matching, sample collection, offset/jitter estimation and stats remain 1.2.4; device-side replies remain 1.4.x. |
