...
| PR-001..003, PR-006 | Partial | Rust side, UDP (task 1.1.3a): `native/vcam-protocol/src/endpoint.rs` (header, HMAC-SHA256/64 trailer with direction keys, `Endpoint::seal/open`) and `message.rs` (`POSE`, `CONTROL_STATE`, `CLOCK`, `STATUS`). Passes `testdata/vcp/messages.json` byte-exactly both ways (`tests/golden.rs`). Still to do: TCP pairing/session (1.1.3b), Swift (1.1.4). |
