...
- **DNS-SD** (NET-001): the host advertises `_vcam-ctl._tcp` for the control port. The TXT record contains `vcp=1` (highest supported protocol version), `blend=<.blend file name>`, and `host=<machine name>`. (`_vcam._udp` is also advertised as NET-001 requires; the device uses the UDP port from `SESSION_CHALLENGE`, not from DNS-SD.)
