...
with open(os.path.join(ROOT, "testdata", "rig", "rig_cases.json"), encoding="utf-8") as f:
    rig_cases = {c["name"]: c for c in json.load(f)["cases"]}
...
assert fields["control_ack"] == "1", done  # the fake's constant CONTROL_STATE, state_seq 1
...
print(f"VCAM_ADDON_APPLY_OK keyposes={len(seen)} max_err={worst:.2e} rig_err={rig_err:.2e} "
